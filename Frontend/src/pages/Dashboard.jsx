...
useEffect(() => {
  rooms.forEach(room => fetchInquiriesForRoom(room._id));
}, [rooms]);
...
{room.photos?.length > 0 && (
  ...
  {Object.keys(inquiries).length === 0 ? (
    ...
    Object.entries(inquiries).map(([roomId, inquiryList]) => (
      <div key={roomId} className="inquiry-section">
        <h4>For: {rooms.find(r => r._id === roomId)?.title}</h4>
        {inquiryList.map(inquiry => (
          <div key={inquiry._id} className="inquiry-card">
            <p><strong>{inquiry.renter.name}</strong></p>
            <p>Email: {inquiry.renter.email}</p>
            <p className="status">Status: {inquiry.status}</p>
